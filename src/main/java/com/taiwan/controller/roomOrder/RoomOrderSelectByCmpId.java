...
@WebServlet("/roomOrder/selectByCmpId")
public class RoomOrderSelectByCmpId extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Integer cmpId = null;
			// 確認用戶有沒有輸入東西
			if (cmpIdString == null || cmpIdString.trim().equals("")) {
				errorMsgs.put("cmpId", "查詢框裡不能空白");
			} else {
				// 確認用戶是不是輸入數字
				try {
					// 字串轉成Integer
					cmpId = Integer.valueOf(cmpIdString);
				} catch (Exception e) {
					errorMsgs.put("cmpIdChange", "請輸入數字");
				}
			}
