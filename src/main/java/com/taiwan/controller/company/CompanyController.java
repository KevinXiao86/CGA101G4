...
//	@ResponseBody
	...
	public String getAllRoomtypesByPage(Model model, HttpSession session,
			...
			@RequestParam(value = "pageSize", required = false)String pageSizeStr, @RequestParam("cmpId")String cmpIdStr) {
		// 1.接收請求參數 pageNo 和 pageSize, 並轉成int類型
		// 默認就是第一頁
		int pageNo = CommonUtils.parseInt(pageNoStr, 1);
		// 默認每頁顯示數量就是4
	...
	public String editCompany(HttpServletRequest request, HttpServletResponse response, Model model, HttpSession session, Company newCompany,
			@RequestParam("cmpId")String cmpId, @RequestParam("headBank")String headBank, @RequestParam("endBank")String endBank,
			@RequestParam("city")String city, @RequestParam("town")String town, @RequestParam("road")String road)throws IOException {
	...
	//註冊
	@PostMapping("/regist")
	public String regist(@RequestParam("serialNo") MultipartFile serialNo, HttpServletRequest request, Model model, HttpSession session
		, @RequestParam("cmpName")String cmpName, @RequestParam("cmpTel")String cmpTel, @RequestParam("headBank")String headBank
		, @RequestParam("endBank")String endBank, @RequestParam("cmpMail")String cmpMail, @RequestParam("cmper")String cmper
		, @RequestParam("cmperTel")String cmperTel, @RequestParam("cmpAccount")String cmpAccount, @RequestParam("cmpPassword")String cmpPassword
		, @RequestParam("cmpIntroduce")String cmpIntroduce, @RequestParam("checkinTime")String checkinTime, @RequestParam("checkoutTime")String checkoutTime
		, @RequestParam("city")String city,	@RequestParam("town")String town, @RequestParam("road")String road
		, @RequestParam("notice")String notice, @RequestParam("canx")String canx) throws IllegalStateException, IOException {
		String bankAccount = headBank + "-" + endBank;
		String location = city + town + road;
